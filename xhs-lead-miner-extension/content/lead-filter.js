/**
 * 线索分层规则筛选（内容脚本内使用，无 ES module 依赖）
 *
 * 流程：硬排除 → must-have 组合 → 加减分 → 阈值入库
 */
(function initLeadFilter() {
  if (window.__XHS_LEAD_FILTER__) return;
  ...
  /** 方法论/清单体标题模式，例：「找海外红人的四种渠道」「海外红人去哪找？别再…」 */
  const CONTENT_FRAME_PATTERNS = [
  ...
  /** 高确信度内容号标题（误杀风险低），用于 AI 前「安全预筛」 */
  const SAFE_SPAM_PATTERNS = [
    /\d+\s*种(渠道|方法|方式|途径|技巧|策略)/,
    /(几|多|数)\s*种(渠道|方法|方式|途径)/,
    /去哪(里)?找/,
    /别再.{0,12}(搜|找|只会)/,
  ...
  /**
   * 供给方自推广告（服务商招客），不是品牌需求方
   * 例：「#出海网红营销就找xxx」「出海投放找我们」
   * 注意：不拦「出海就找达人」这类品牌需求表述
   */
  const SUPPLY_AD_PATTERNS = [
  ...
  const CERTAIN_NOISE_WORDS = [
    '求职', '招聘', '兼职', '培训班', '训练营', '变现课', '代运营教学',
    '接单教学', '陪跑', '免费领资料', '资料领取', '割韭菜', '代理招商', '加盟火热',
    '就找我们', '找我们合作', '承接品牌', '欢迎品牌方',
  ];

  /** 强采购意图：品牌在找资源（注意：不含「找我们/就找」类供给话术） */
  const PURCHASE_RESCUE_WORDS = [
  ...
  const AUTHOR_NOISE_WORDS = [
    '讲师', '教练', '培训', '副业', '代运营', '课程', '陪跑', '导师',
  ];
  ...
  function hasPurchaseRescue(text) {
    // 供给广告优先，不因「合作」等词误放行
  ...
  /**
   * AI 预筛：smart / off / safe / strict
   * safe 也会挡「就找xxx」类供给广告
   */
  function shouldDropBeforeAi(text, extraExclude = [], mode = 'smart', keyword = '') {
    if (!mode || mode === 'off') {
      // off 也挡极明显服务商自推，避免污染待确认
  ...
  function scoreLead(text, keyword, signals, authorName) {
    ...
    if (matched.intent.length >= 2) {
      score += 8;
    }

    const authorNoise = hitWords(authorName || '', AUTHOR_NOISE_WORDS);
    if (authorNoise.length) {
      score -= 20;
    }
  ...
  function evaluateLead({
    ...
    const { score, tier, matched } = scoreLead(
      fullText,
      keyword,
      mustHave.signals,
      authorName,
